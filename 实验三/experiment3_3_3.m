%experiment3_3_3
%用高通滤波器选出原信号st中以1000Hz为载频，200Hz为带宽的分量
clear,clc,clf
...
%===高通巴特沃斯数字滤波器
fp=890;fs=550;%高通滤波器的指标
wpz=fp*2*pi*T;wsz=fs*2*pi*T;%设计HF要求的数字指标，wpz是数字通带截止频率，wsz是数字阻带截止频率
%===设置双线性变换法的滤波器参数
...
[B,A]=butter(M,wc,'high','s');%计算高通滤波器系统函数Hs分子分母多项式系数
%===用双线性变换法转换成数字滤波器
...
subplot(211)
plot(t,yt);grid on
xlabel('t/s');ylabel('幅度');title('原信号st通过高通滤波器的结果时域')
axis([0,Tp/8,min(st),max(st)])
subplot(212)
stem(f,abs(yw)/max(abs(yw)),'.');grid on
xlabel('f/Hz');ylabel('幅度');title('原信号st通过高通滤波器的结果频域')
axis([0,1200,0,1.2])